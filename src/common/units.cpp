...
std::string format_bandwidth(double bits_per_second) {
    ...
    if (bits_per_second >= 1e9) {
        ...
        std::snprintf(buf, sizeof(buf), "%7.2f Kbits/sec", bits_per_second / 1e3);
    } else {
        std::snprintf(buf, sizeof(buf), "%7.2f bits/sec", bits_per_second);
    }
...
std::string format_bytes(uint64_t bytes) {
    ...
    if (bytes >= (1ULL << 30)) {
        ...
        std::snprintf(buf, sizeof(buf), "%7.2f KBytes", static_cast<double>(bytes) / (1ULL << 10));
    } else {
        std::snprintf(buf, sizeof(buf), "%7llu Bytes", static_cast<unsigned long long>(bytes));
    }
